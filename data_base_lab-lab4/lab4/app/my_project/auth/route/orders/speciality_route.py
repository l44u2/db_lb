...
@swag_from({
    ...
    'responses': {200: {'description': 'Speciality'}, 404: {'description': 'Not found'}}
})
def get_speciality(speciality_id: int) -> Response:
    ...
    speciality = {"id": row[0], "speciality_type": row[1]}
    return make_response(jsonify(speciality), HTTPStatus.OK)
...
@swag_from({
    'tags': ['Speciality'],
    'summary': 'Partially update speciality',
    'parameters': [
        {'name': 'speciality_id', 'in': 'path', 'type': 'integer', 'required': True},
        {'name': 'body', 'in': 'body', 'required': True, 'schema': {
            'type': 'object',
